...
__device__ glm::vec3 computeColorFromSH_forward(int idx, int deg, int max_coeffs, const glm::vec3* means, glm::vec3 campos, const float* shs)
{
...
__device__ void computeColorFromSH(
	...
	const float* shs, 
	const bool* clamped, 
	const glm::vec3* dL_dcolor, // 来自上层的颜色梯度
	glm::vec3* dL_dmeans, 
	glm::vec3* dL_dshs)  //输出
{
	...
	glm::vec3* dL_dBs)
{
	...
	if (shs){
		...
		glm::vec3 trans_D = glm::exp(-beta_D * depth);
		glm::vec3 trans_B = glm::exp(-beta_B * depth);

		glm::vec3 dL_dresult = dL_dRGB * trans_D;

		// 每通道分别计算梯度
		glm::vec3 dL_dbeta_D(0.0f), dL_dbeta_B(0.0f), dL_dB(0.0f);
		for (int c = 0; c < 3; ++c) {
			dL_dbeta_D[c] = dL_dRGB[c] * result[c] * (-depth * trans_D[c]);
			dL_dbeta_B[c] = dL_dRGB[c] * (-B[c]) * (-depth * trans_B[c]);
			dL_dB[c]      = dL_dRGB[c] * (1.0f - trans_B[c]);
		}
		
		atomicAdd(&dL_dbeta_Ds[idx].x, dL_dbeta_D.x);
		atomicAdd(&dL_dbeta_Ds[idx].y, dL_dbeta_D.y);
		atomicAdd(&dL_dbeta_Ds[idx].z, dL_dbeta_D.z);

		atomicAdd(&dL_dbeta_Bs[idx].x, dL_dbeta_B.x);
		atomicAdd(&dL_dbeta_Bs[idx].y, dL_dbeta_B.y);
		atomicAdd(&dL_dbeta_Bs[idx].z, dL_dbeta_B.z);

		atomicAdd(&dL_dBs[idx].x, dL_dB.x);
		atomicAdd(&dL_dBs[idx].y, dL_dB.y);
		atomicAdd(&dL_dBs[idx].z, dL_dB.z);

		// 修正颜色梯度以正确传递到原始颜色/SH系数
		for (int c = 0; c < 3; ++c) {
			((glm::vec3*)dL_dcolor)[idx][c] *= trans_D[c];
		}
...
)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			// // new
			// float dL_dx = dL_dG * dG_ddelx * ddelx_dx;
			// float dL_dy = dL_dG * dG_ddely * ddely_dy;
			// atomicAdd(&dL_dmean2D[global_id].x, dL_dx);
			// atomicAdd(&dL_dmean2D[global_id].y, dL_dy);
			// float dL_norm = sqrt(dL_dx * dL_dx + dL_dy * dL_dy);
			// atomicAdd(&dL_dmean2D[global_id].z, dL_norm);
